...
export const OwnerHomeCon: FC = () => {
  ...
  const onClickCategory = (category: CategoryType) => {
    ...
    if (category.id === 0) {
      ...
      const filteredMenu = allMenus.filter(
        (item: MenuItemType) => item.category === category.name
      );
  ...
  return (
    <OwnerHomePre
      ...
      onConfirmDialogNegative={onClickConfirmCalcel}
      onConfirmDialogPositive={onClickConfirmDelete}
    />
  );
};
